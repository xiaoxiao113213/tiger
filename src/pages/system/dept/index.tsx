...
import { deptDeleteApi, deptPageAndAccountApi } from '@/pages/system/dept/api.tsx';
...
export default () => {
  ...
  const openDetailFn = (record: DeptVo) => {
    setDetailIdFn(record.id);
    setDetailModalFn(OperateEnum.detail);
  };
  const openAddOrUpdateFn = (record?: DeptVo) => {
  ...
  const deleteDetailFn = async (item: DeptVo) => {
    Modal?.confirm({
      title: '是否确认删除',
      // content: item.account,
      icon: null,
      onOk: async () => {
        let rst = await deptDeleteApi({ id: item.id });
  ...
  const columns: ProColumns<DeptVo>[] = [
    ...
    },
    {
      ...
      render: (text, record, _, action) => [
        <a key="detail" onClick={() => openDetailFn(record)} hidden={record.type === 1}>
          详情
        </a>,
        <a key="log" onClick={() => openAddOrUpdateFn(record)} hidden={oCheckHidden('dept:edit') || record.type === 1}>
          编辑
        </a>,
        <a key="rebuild" onClick={() => deleteDetailFn(record)} hidden={oCheckHidden('dept:del')}>
          删除
        </a>,
        // <TableDropdown
        //     key="actionGroup"
    ...
    <div>
      <ProTable<DeptVo>
        ...
        request={async (params = {}, sort, filter) => {
          ...
          let rst = await deptPageAndAccountApi({ ...params, ...p });
          return {
            data: rst.data,
            // success 请返回 true，
      ...
      <Drawer
        title={addOrUpdateModal == OperateEnum.edit ? '部门编辑' : '部门新增'}
