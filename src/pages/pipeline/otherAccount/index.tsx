...
import { otherAccountDeleteApi, otherAccountPageApi } from './api';
import { OtherAccountListVo, OtherAccountVo } from './ApiBo';
...
export default () => {
  ...
  const openDetailFn = (record: OtherAccountListVo) => {
    setDetailIdFn(record.id);
    setDetailModalFn(OperateEnum.detail);
  };
  const openAddOrUpdateFn = (record?: OtherAccountListVo) => {
    setDetailIdFn(record?.id);
    if (record) {
  ...
  const deleteDetailFn = async (item: OtherAccountListVo) => {
    Modal?.confirm({
      title: '是否确认删除',
      // content: item.account,
      icon: null,
      onOk: async () => {
        let rst = await otherAccountDeleteApi({ id: item.id });
  ...
  const columns: ProColumns<OtherAccountListVo>[] = [

    {
      ...
      copyable: false,
      search: false,
      valueEnum: OtherAccountTypeValueEnum,
    },
    {
      title: '账号名称',
    ...
    <div>
      <ProTable<OtherAccountListVo>
        ...
        request={async (params = {}, sort, filter) => {
          ...
          // @ts-ignore
          let p: OtherAccountVo = { pageNum: params.current, pageSize: params.pageSize, pageSort: sort };
          // console.log("参数", params)
          let rst = await otherAccountPageApi({ ...params, ...p });
          return {
            data: rst.data.list,
            // success 请返回 true，
        ...
        columnsState={{
          persistenceKey: 'otherAccount-pro-table',
