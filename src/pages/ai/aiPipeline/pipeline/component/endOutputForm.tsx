...
const EndOutputForm = (props: {
  ...
  const EditableCell: React.FC<React.PropsWithChildren<EditableCellProps>> = (
    ...
    if (title == '变量名') {
      if (record.name === 'output') {
        return <td class="ant-table-cell">{record.name}</td>;
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: DataType) => {
        const editable = record.name === 'output';
        if (editable) {
          return <></>;
        }
