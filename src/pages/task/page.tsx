...
import { Button, Drawer, message, Modal, Progress } from 'antd';
import { PlusOutlined } from '@ant-design/icons';
import Detail from './detail';
import AddOrUpdate from './addOrUpdate';
import { taskDeleteApi, taskPageApi } from './api';
import { TaskListVo, TaskVo } from './ApiBo';
import { OperateEnum } from '@/utils/enum.ts';
import { checkApiRst } from '@/utils/utils.ts';

export default (props: { tagNum: string }) => {
  ...
  const openDetailFn = (record: TaskListVo) => {
    setDetailIdFn(record.taskId);
    setDetailModalFn(OperateEnum.detail);
  };
  const openAddOrUpdateFn = (record?: TaskListVo) => {
    setDetailIdFn(record?.taskId);
    if (record) {
  ...
  const deleteDetailFn = async (item: TaskListVo) => {
    Modal?.confirm({
      title: '是否确认删除',
      // content: item.account,
      icon: null,
      onOk: async () => {
        let rst = await taskDeleteApi({ taskId: item.taskId });
  ...
  }, [props.tagNum]); // 第二个参数表示依赖项
  const columns: ProColumns<TaskListVo>[] = [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        return <Progress type="circle" percent={text} size={'small'} />;
      },
    },
    // {
    //   title: '依赖',
    ...
    <div>
      <ProTable<TaskListVo>
        ...
        request={async (params = {}, sort, filter) => {
          ...
          // @ts-ignore
          let p: TaskVo = { pageNum: params.current, pageSize: params.pageSize, pageSort: sort };
          // console.log("参数", params)
          let rst = await taskPageApi({ ...params, ...p });
          return {
            data: rst.data.list,
            // success 请返回 true，
