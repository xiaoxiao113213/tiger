...
import { DocDetailVo } from './ApiBo';
import { docGetOneApi, docSaveApi, docUpdateApi } from './api';
...
const AddOrUpdate = (props: Props) => {
  const [formRef] = Form.useForm<DocDetailVo>();
  const [detail, setDetailFn] = useState<DocDetailVo>(getNullBo);

  const initMyDataFn = async () => {
    if (props.operateEnum === OperateEnum.edit) {
      let rst = await docGetOneApi({ docId: props.detailId });
  ...
  const saveFn = async () => {
    ...
    let res: Rst<any>;
    if (props.operateEnum === OperateEnum.edit) {
      res = await docUpdateApi({ ...values, docId: props.detailId });
    } else {
      res = await docSaveApi({ ...values });
    }
