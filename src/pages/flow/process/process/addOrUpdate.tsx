...
import { processGetOneApi, processSaveApi, processUpdateApi } from './api/processApi.tsx';
...
const AddOrUpdate = (props: Props) => {
  const [formRef] = Form.useForm<ProcessDetailVo>();
  ...
  const saveFn = async () => {
    ...
    let res: Rst<any>;
    if (props.operateEnum === OperateEnum.edit) {
      res = await processUpdateApi({ ...values, processId: props.detailId });
    } else {
      res = await processSaveApi({ ...values });
    }
