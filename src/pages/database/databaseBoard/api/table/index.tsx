...
import { tableDeleteApi, tablePageApi } from './api';
import { TableListVo, TableVo } from './ApiBo';
...
export default () => {
  ...
  const openDetailFn = (record: TableListVo) => {
    setDetailIdFn(record.tableId);
    setDetailModalFn(OperateEnum.detail);
  };
  const openAddOrUpdateFn = (record?: TableListVo) => {
    setDetailIdFn(record?.tableId);
    if (record) {
  ...
  const deleteDetailFn = async (item: TableListVo) => {
    Modal?.confirm({
      title: '是否确认删除',
      // content: item.account,
      icon: null,
      onOk: async () => {
        let rst = await tableDeleteApi({ tableId: item.tableId });
  ...
  const columns: ProColumns<TableListVo>[] = [
    {
      title: 'tableId',
      dataIndex: 'tableId',
      valueType: 'text',
    ...
    <div>
      <ProTable<TableListVo>
        ...
        request={async (params = {}, sort, filter) => {
          ...
          // @ts-ignore
          let p: TableVo = { pageNum: params.current, pageSize: params.pageSize, pageSort: sort };
          // console.log("参数", params)
          let rst = await tablePageApi({ ...params, ...p });
          return {
            data: rst.data.list,
            // success 请返回 true，
        ...
        columnsState={{
          persistenceKey: 'table-pro-table',
