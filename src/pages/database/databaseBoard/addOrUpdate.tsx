...
import { databaseBoardGetOneApi, databaseBoardSaveApi, databaseBoardUpdateApi } from './api';
...
const AddOrUpdate = (props: Props) => {
  ...
  const saveFn = async () => {
    ...
    let res: Rst<any>;
    if (props.operateEnum === OperateEnum.edit) {
      res = await databaseBoardUpdateApi({ ...values, databaseBoardId: props.detailId });
    } else {
      res = await databaseBoardSaveApi({ ...values });
    }
